        ...
        <>
            ...
            <Modal open={isModelOpen} onClose={handleCloseRow}>
                <Box className='modelBg' sx={{
                    ...
                    <Form>
                        {isNewAgent ? (
                            <h4 className='d-flex justify-content-center'>Add new Agent Details</h4>
                        ) : (
                            <h4 className='d-flex justify-content-center'>Edit Agent Details</h4>
                        )}
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.username : '')}
                            label="Username"
                            fullWidth
                        />
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.firstName : '')}
                            label="First Name"
                            fullWidth
                        />
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.lastName : '')}
                            label="Last Name"
                            fullWidth
                        />
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.email : '')}
                            label="Email"
                            fullWidth
                        />
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.userStatus : '')}
                            label="User Status"
                            fullWidth
                        />
                        <TextField
                            className='mb-2'
                            value={isNewAgent ? '' : (selectedRowData ? selectedRowData.assignedCategory : '')}
                            label="Assigned Category"
                            fullWidth
                        />
                        <Button variant="contained" color="primary" onClick={handleSaveAgent} onChange={(e) => e.target.value}>
