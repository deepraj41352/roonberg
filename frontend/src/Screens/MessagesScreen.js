...
function MessagesScreen() {
    ...
    const [allReceivedUsers, setAllReceivedUsers] = useState();
    useEffect(() => {
        const fetchReceiverUsers = async () => {
            try {
                const { data } = await axios.get(`/api/user`);
                console.log("allReceivedUsers", data);
                setAllReceivedUsers(data);
            } catch (error) {
                console.log(error);
            }
        };
    ...
    useEffect(() => {
        const fetchUsersConversation = async () => {
            try {
                const { data } = await axios.get(`/api/conversation/${userInfo._id}`);
                console.log("Original data", data);

                const updatedUsers = data.map((conversation) => {
                    console.log("Original conversation", conversation);

                    const updatedMembers = conversation.members
                        .filter((memberId) => memberId !== userInfo._id)
                        .map((memberId) => {
                            const receiver = allReceivedUsers.find((receiveId) => receiveId._id === memberId);
        ...
        fetchUsersConversation();
    }, [allReceivedUsers, userInfo._id]);
    ...
    const getMessages = async (converId) => {
        try {

            const { data } = await axios.get(`/api/message/${converId}`);
    ...
    />
                ...
                <Card className="chatWindowProjectInfo">
                    ...
                    {users && users.map((conversation) => (
                        <div key={conversation._id}>
                            {conversation.members.map((member) => (
                                <div className='d-flex align-items-start p-2 gap-2' key={member} onClick={() => getMessages(conversation._id)}>
                                    <Avatar src={member.profile_picture} />
                                    <CardText>{member.first_name}</CardText>
